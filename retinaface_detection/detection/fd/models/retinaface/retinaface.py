...
sys.path.append("./detection/fd/models/retinaface/rcnn/")
...
from .rcnn.processing.nms import (
    cpu_nms_wrapper,
    gpu_nms_wrapper,
)
...
class RetinaFace:
    def __init__(
        ...
        self.model.bind(
            data_shapes=[("data", (1, 3, self.image_h, self.image_w))],
            for_training=False,
        )
    ...
    def get_input(self, img):
        ...
        for i in range(3):
            im_tensor[0, i, :, :] = (
                im[:, :, 2 - i] / self.pixel_scale - self.pixel_means[2 - i]
            ) / self.pixel_stds[2 - i]
        # if self.debug:
        ...
        for img in imgs:
            for im_scale in scales:
                for flip in flips:
                    ...
                    data = nd.array(im_tensor)
                    db = mx.io.DataBatch(
                        data=(data,), provide_data=[("data", data.shape)]
                    )
                    if self.debug:
                        timeb = datetime.datetime.now()
                        diff = timeb - timea
                            ...
                            for diff_idx in __idx:
                                ...
                                if body.shape[1] // A == 2:  # cls branch
                                    ...
                                    cascade_deltas = body.transpose(
                                        (0, 2, 3, 1)
                                    ).reshape((-1, bbox_pred_len))
                                    cascade_deltas[:, 0::4] = (
                                        cascade_deltas[:, 0::4] * self.bbox_stds[0]
                                    )
                                    cascade_deltas[:, 1::4] = (
                                        cascade_deltas[:, 1::4] * self.bbox_stds[1]
                                    )
                                    cascade_deltas[:, 2::4] = (
                                        cascade_deltas[:, 2::4] * self.bbox_stds[2]
                                    )
                                    cascade_deltas[:, 3::4] = (
                                        cascade_deltas[:, 3::4] * self.bbox_stds[3]
                                    )
